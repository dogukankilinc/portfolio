...
if (parts.length === 2) {
    ...
    parts[0] = parts[0].replace(/exp1Desc:\s*'.*?',/s, `expFiges1: ${enFiges1},`);
    parts[0] = parts[0].replace(/exp2Desc:\s*'.*?',/s, `expFiges2: ${enFiges2},`);
    parts[0] = parts[0].replace(/exp3Desc:\s*'.*?',/s, `expFiges3: ${enFiges3},`);
    
    // Turkish replacements
    parts[1] = parts[1].replace(/exp1Desc:\s*'.*?',/s, `expFiges1: ${trFiges1},`);
    parts[1] = parts[1].replace(/exp2Desc:\s*'.*?',/s, `expFiges2: ${trFiges2},`);
    parts[1] = parts[1].replace(/exp3Desc:\s*'.*?',/s, `expFiges3: ${trFiges3},`);
    
    js = parts[0] + 'tr: {' + parts[1];
    fs.writeFileSync('js/main.js', js);
    console.log('main.js updated with correct keys and bullet lists.');
} else {
